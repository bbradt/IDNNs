...
def build_model(activation_function, layerSize, input_size, num_of_classes, learning_rate_local, save_file, covn_net, l1=0.0, l2=0.0):
    ...
    model = mo.Model(input_size, layerSize, num_of_classes, learning_rate_local, save_file, int(activation_function),
                     cov_net=covn_net, l1_lambda=l1, l2_lambda=l2)
    return model
...
def train_network(layerSize, num_of_ephocs, learning_rate_local, batch_size, indexes, save_grads,
    ...
    grad_l1, grad_l2 = [[[]] * len(indexes) for _ in range(2)]
    ...
    # Build the network
    model = build_model(model_type, layerSize, input_size, num_of_classes, learning_rate_local, name, covn_net, l1=l1, l2=l2)
    ...
    network = {}
    network['ws'] = ws
    network['test_prediction'] = [1.0 - t for t in test_prediction]
    network['train_prediction'] = [1.0 - t for t in train_prediction]
    network['loss_test'] = loss_func_test
    network['loss_train'] = loss_func_train
    network['l1_norm'] = grad_l1
    network['l2_norm'] = grad_l2
